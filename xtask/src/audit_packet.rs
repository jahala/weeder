...
//!
//! What the packet seals is the judged content and nothing else: the rule ids,
//! each finding's level, path and line, and the commit's hunks. A finding's
//! sentence is left out, because the question put to an auditor is whether the
//! rule's claim is true of the hunk, and the sentence carries wording that
//! moves for reasons the judgement has nothing to do with. It moved once: the
//! tool's rename changed the suppression token a T2 message teaches, ten kept
//! packets stopped regenerating, and the price of a re-seal is forty fresh
//! auditor sessions. Every recorded answer names its packet's SHA-256 and
//! `scripts/check/audit-fairness.sh` regenerates the kept packets and compares
//! them byte for byte, so a word added to this file costs that again. The rule
//! catalogue's one-line claim stays: it is what the auditor judges against.
...
fn write_case_packets(
    ...
    for case in material.blocked {
        ...
        let whole = diff(scratch.path(), &parent, &sha, &[])?;
        // Where the whole change is over the cap, the packet falls back to the
        // diff of the files the findings name, which is the change the auditor
        // is being asked about. The file table above it still counts the whole.
        let shown = if whole.len() > DIFF_CAP_BYTES {
            let named: Vec<String> = finding_paths(&findings);
            match diff(scratch.path(), &parent, &sha, &named) {
                Ok(narrowed) if !named.is_empty() && narrowed.len() <= DIFF_CAP_BYTES => {
                    Shown::Named(narrowed)
                }
                _ => Shown::None,
            }
        } else {
            Shown::Whole(whole.clone())
        };
        let body = case_packet(&Packet {
            seed: &request.seed,
            case: &format!("blocked:{}:{}", case.repo, case.sha),
            summary: &format!("Rules: {}", case.rules.join(", ")),
            catalogue: &catalogue_lines(&rules_table, &case.rules),
    ...
    for case in material.recall {
        let repo = material
            ...
            .get(&case.repo)
            .ok_or_else(|| format!("{} is not named by the corpus files", case.repo))?;
        let replay = mutate::replay_case_in(
            &material.replay_cache,
            repo,
            &case.sha,
            &case.rule,
            &case.lang,
        ...
        let extra = format!(
            "Planted site: {}\n\nPlanted shape: {}\n\nQuestion: is rule {}'s shape genuinely present at that planted site, and did weeder report it there?",
        ...
        let body = case_packet(&Packet {
            ...
            summary: &format!("Rule: {}", case.rule),
            catalogue: &catalogue_lines(&rules_table, std::slice::from_ref(&case.rule)),
            findings: &findings_table(&replay.findings),
            extra: Some(&extra),
            whole: &replay.diff,
            shown: &shown(&replay.diff),
        });
        fs::write(
            dir.join(format!(
                "{}.md",
...
fn material(
    ...
    Ok(PacketMaterial {
        ...
        repo_defs,
        fetched,
        replay_cache: scratch.path().join("recall-replay"),
        _scratch: scratch,
    })
}
...
fn packet(request: &Request, root: &Path, report: &str) -> Result<String, Box<dyn Error>> {
    ...
    let replay_cache = scratch.path().join("recall-replay");
    out.push_str("# calibration audit blind packet, 2026-09\n\n");
    out.push_str("This packet was generated by `cargo xtask audit-packet` from the pinned corpus. It contains case identifiers, rule catalogue text, weeder's findings as rule id, level, path and line, verbatim `git diff` output and recall planting questions only. It does not contain judgement rows or audit verdicts.\n\n");
    ...
    for case in recall {
        let repo = repo_defs
            ...
            .ok_or_else(|| format!("{} is not named by the corpus files", case.repo))?;
        let replay =
            mutate::replay_case_in(&replay_cache, repo, &case.sha, &case.rule, &case.lang)?;
        out.push_str(&format!(
            "### recall:{}:{}:{}:{}:{}\n\nRule: {}\n\n{}\n{}\nPlanted site: {}\n\nPlanted shape: {}\n\nQuestion: is rule {}'s shape genuinely present at that planted site, and did weeder report it there?\n\n```diff\n{}\n```\n\n",
            case.rule,
            case.lang,
...
/// What weeder reported, as the judged content alone.
///
/// A finding is a rule id, a level, a path and a line; its sentence is the same
/// claim written for a person, and it is left out. An auditor is asked whether
/// the rule's claim, printed once in the catalogue above this table, is true of
/// the hunks below it, and the sentence adds nothing to that question while
/// binding the packet's hash to wording that changes.
fn findings_table(findings: &[calibrate::Finding]) -> String {
    let mut out =
        String::from("Weeder findings:\n\n| Rule | Level | Path | Line |\n|---|---|---|---:|\n");
    if findings.is_empty() {
        out.push_str("| none | none |  | 0 |\n\n");
        return out;
    }
    for finding in findings {
        out.push_str(&format!(
            "| {} | {} | `{}` | {} |\n",
            finding.rule, finding.level, finding.path, finding.line,
        ));
    }
    out.push('\n');
    out
...
/// What a packet can show of the change: the whole diff, the diff of the files
/// the findings name, or nothing but the file table.
///
/// A packet is one session's whole input, and a session has a context window, so
/// the diff has a cap. What the cap must not do is hand an auditor a rule's
/// claim about a file and withhold that file: a verdict taken on a withheld
/// change is a guess, and a guess counted as agreement or disagreement is a
...
/// The whole diff where it fits, and nothing where it does not. Recall packets
/// carry one replayed site, which is already narrow.
fn shown(diff: &str) -> Shown {
    if diff.len() > DIFF_CAP_BYTES {
        Shown::None
    } else {
        Shown::Whole(diff.to_string())
    }
}

/// Everything one case packet is written from.
struct Packet<'a> {
    seed: &'a str,
    case: &'a str,
    summary: &'a str,
    ...
    findings: &'a str,
    extra: Option<&'a str>,
    /// The whole change. It decides whether the cap was reached, and the file
    /// table is counted off it whatever the packet ends up showing.
    whole: &'a str,
    shown: &'a Shown,
}

fn case_packet(packet: &Packet) -> String {
    let Packet {
        seed,
        case,
        summary,
    ...
    out.push_str(&format!(
        "Diff shown: {}\n\n",
        match shown {
            Shown::Whole(_) => "the whole change",
            Shown::Named(_) => "the files the findings name",
            Shown::None => "none",
        }
    ));
    ...
    match shown {
        ...
        Shown::Named(text) => {
            out.push_str("The whole git diff for this case exceeds the stated cap, so the generator cut it to the files the findings name rather than cutting it mid-line. Every other file the change touched is counted in the table above.\n\n");
            out.push_str("```diff\n");
            out.push_str(text);
            if !text.ends_with('\n') {
                out.push('\n');
        ...
        Shown::None => {
            out.push_str("```text\n");
            out.push_str("The full git diff for this case exceeds the stated cap, and so does the diff of the files the findings name, so the generator omitted it instead of cutting it silently. The table above is what the change did to every file it touched, counted from those same bytes.\n");
            out.push_str("```\n");
        }
    }
    out
...
/// What the change did to every file it touched, counted from the diff itself.
///
/// A packet whose diff is over the cap carries no diff, and a rule's claim about
/// one file cannot be judged without knowing what happened to the rest: a test
/// file deleted beside the code it tested is a different change from the same
/// file deleted alone. The count is read off the same bytes the diff would have
/// carried, so it needs no second question to git and cannot disagree with what
/// the packet shows.
fn touched(diff: &str) -> String {
    let mut files: Vec<(String, usize, usize)> = Vec::new();
    // A file's header carries `---` and `+++` lines that are not content, and a
    // removed line whose text begins with `---` looks exactly like one. What
    // tells them apart is the first `@@` of the file: everything before it is
    // header, everything after it is the change.
    let mut in_hunk = false;
    for line in diff.lines() {
        if let Some(rest) = line.strip_prefix("diff --git ") {
            files.push((headed(rest), 0, 0));
            in_hunk = false;
            continue;
        }
        if line.starts_with("@@") {
        ...
        let Some((_, added, deleted)) = files.last_mut() else {
            continue;
        };
        if !in_hunk {
            continue;
...
/// The path a `diff --git a/<path> b/<path>` header names, taken from its second
/// half, so a rename reads as the name the file has after the change.
fn headed(rest: &str) -> String {
    let halves: Vec<&str> = rest.split(" b/").collect();
    match halves.len() {
        0 | 1 => rest.trim_start_matches("a/").to_string(),
        _ => halves[halves.len() - 1].to_string(),
    }
}
...
mod tests {
    ...
    fn finding(rule: &str, message: &str) -> calibrate::Finding {
        calibrate::Finding {
            rule: rule.to_string(),
            level: "error".to_string(),
            path: "test/renderer.test.ts".to_string(),
            line: 22,
            message: message.to_string(),
        }
    }

    /// The seal is the judged content, so a message rewritten between two runs
    /// leaves the packet where it was. This is the rename measured: the token a
    /// T2 message teaches changed spelling, ten kept packets stopped
    /// regenerating, and forty auditor sessions were bought back.
    #[test]
    fn a_rewritten_message_does_not_move_the_packet() {
        let before = findings_table(&[finding(
            "T2",
            "1 assertion went out of this file. carry a `Weed-allow: T2` trailer.",
        )]);
        let after = findings_table(&[finding(
            "T2",
            "1 assertion went out of this file. carry a `Weeder-allow: T2` trailer.",
        )]);
        assert_eq!(
            before, after,
            "a finding's wording must not reach the packet the auditors' answers are sealed to"
        );
    }

    /// What the table does carry: the rule id, the level, the path and the line,
    /// which is what the auditor's question is about.
    #[test]
    fn the_table_carries_the_judged_content() {
        let table = findings_table(&[finding("T2", "some sentence about assertions")]);
        assert!(table.contains("| Rule | Level | Path | Line |"), "{table}");
        assert!(
            table.contains("| T2 | error | `test/renderer.test.ts` | 22 |"),
            "{table}"
        );
        assert!(
            !table.contains("sentence"),
            "the finding's sentence reached the packet:\n{table}"
        );
    }
}
