...
- The suppression tokens are `Weed-allow:` and `weed-allow <RULE>:`, and they keep the name the tool carried before the rename to weeder. The finding messages that teach them are printed into the blind audit's case packets, `scripts/check/audit-fairness.sh` regenerates those packets and compares them byte for byte, and each of the forty recorded auditor answers opens with its packet's SHA-256. Renaming a token breaks every seal and the only honest repair is forty fresh auditor sessions. The reason is written beside the parser in `src/core/suppress.rs`; do not tidy it.
