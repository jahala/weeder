...
//! that is B1.
//!
//! The states are built in a worktree of weeder's own, somewhere under the
...
//! process weeder starts is given a deadline it cannot outlive.
...
/// line the way its author meant it is a shell. weeder still hands it over as a
/// program and an argument array, so the line is data to everything but the
/// shell that was asked to read it.
const SHELL: &str = "sh";

/// How long weeder waits for the test command where the caller names no deadline.
/// A suite is slower than everything else weeder does by orders of magnitude, and
/// a suite that has not answered in five minutes is one nobody is waiting for.
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(300);

/// What the commit weeder applies is called in the worktree it applies it in.
/// Nobody reads these commits; they exist so the next state can be built on top
/// of the one before it.
const TEST_PHASE: &str = "the test phase, as weeder bite applied it";
const IMPLEMENTATION: &str = "the implementation, as weeder bite applied it";
...
fn judge(request: &Request) -> Result<Answer, String> {
    ...
    // weeder's to take away, and a run that walked out around this line would
...
fn changes(root: &Path, pair: &Pair) -> Result<Vec<Change>, String> {
    ...
    if judged.is_empty() {
        return Err(format!(
            "the commit weeder was given as the test phase, {}, changes no file against {}. bite judges a test commit, so name one with --test-commit.",
            short(&pair.test),
            short(&pair.base)
        ));
    }
...
/// The two runs, in the worktree weeder built for them. The first is the one that
...
/// there is the state bite exists to build, so weeder says which one and stops
...
fn apply(at: &Path, commit: &str, message: &str, phase: &str) -> Result<(), String> {
    match git::apply_commit(at, commit, message).map_err(|error| error.to_string())? {
        ...
        git::Applied::Refused { message } => Err(format!(
            "weeder could not apply {} as {phase}: {message}. bite needs a test commit that applies to the base on its own, which is what a conductor that commits its phases separately leaves behind.",
            short(commit)
        )),
    }
}
...
fn command(at: &Path, request: &Request) -> Result<i32, String> {
    ...
    match exec::run(at, SHELL, &["-c", &request.test], timeout) {
        ...
        Err(exec::ExecError::TimedOut { seconds, .. }) => Err(format!(
            "`{}` was still running after {seconds}s and weeder stopped waiting. give --timeout the seconds the suite needs, or name a command that answers in them.",
            request.test
        )),
...
fn worktree_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("weeder-bite-{}-{since}", std::process::id()))
}
