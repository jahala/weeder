...
|---|---|---|---:|---|
| T2 | error | `src/mcp/tools/search.rs` | 115 | 1 assertion went out of this file: it made 3 and now makes 2. a case that checks nothing passes whatever the code does, so the suite reports green over behaviour held by nobody. put the checks back, or carry a `Weed-allow: T2` trailer saying which claim stopped being worth making. |
