...
| T2 | error | `src/index/bloom.rs` | 14 | 6 assertions went out of this file: it made 43 and now makes 37. a case that checks nothing passes whatever the code does, so the suite reports green over behaviour held by nobody. put the checks back, or carry a `Weed-allow: T2` trailer saying which claim stopped being worth making. |
