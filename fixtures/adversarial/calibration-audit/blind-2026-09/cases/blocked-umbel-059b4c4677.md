...
|---|---|---|---:|---|
| T2 | error | `test/unit/providers/codex.test.ts` | 10 | 3 assertions went out of this file: it made 33 and now makes 30. a case that checks nothing passes whatever the code does, so the suite reports green over behaviour held by nobody. put the checks back, or carry a `Weed-allow: T2` trailer saying which claim stopped being worth making. |
