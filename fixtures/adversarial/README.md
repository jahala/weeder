...
deleted. `after/.weeder-commit` is the message of the commit being prepared, for
the rules that read trailers; it never reaches the tree. An ignore file is
written as `weeder.gitignore` and copied in as `.gitignore`: named literally, it
...
`paths/`, X1 under `prose/` for the files weeder has no grammar for.

A fixture whose rule asks git when a line was written states the date its state
was committed on in `before/.weeder-date`, in any spelling git reads. The file
...
weeder does not carry what it refuses, so no file here opens a line with a
...
| placeholder            | marker    |
| ---------------------- | --------- |
| `{{weeder:ours}}`        | the opener |
| `{{weeder:base}}`        | the base side of a diff3 merge |
| `{{weeder:separator}}`   | the divider |
| `{{weeder:theirs}}`      | the closer |

A label follows the placeholder as it follows the marker, so
`{{weeder:ours}} HEAD` becomes the line git wrote. The binary under test reads the
...
The same rule applies to a blob. `{{weeder:binary}}` is expanded into a run of
control bytes opening with a NUL, which is git's own test for a file it cannot
show a diff of and the one weeder asks too. G2's `fire/` fixtures write it, and
...
with weeder's own findings beside them, and the raw record of each auditor session.
They are specimens in the same sense, real diffs that fired a rule, and they live
under this root so that `[scope] specimens` keeps weeder from judging what it
