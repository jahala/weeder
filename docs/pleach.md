...
`weed check --strict` is the whole gate.

```json
"accept": { "smoke": "weed check --strict" }
```
...
With no ref argument weed judges the index plus the working tree against `HEAD`. That is the staged
...
node and a base argument would name the commit weed already compares against.
...
commit message it is preparing, or an inline `weed-allow` comment beside the line weed objected to.
Under `--strict` weed reports those findings at the level their rule carries, so a suppression the
