...
10. **The emitted payload hash covers the whole loop file, Tried included.** Between emitting the recall plan and landing its node I appended a line to the loop's `## Tried` (the append-only memory the law asks conductors to keep). `pleach land` then refused the composition: `tend2 verify … --expect-payload db93f720a625` answered "payload drift: file of record is 6bada776201e, the worktree base is stale (SNAG-13)". The node's work was untouched and its checks unchanged; only a Tried line had moved. Either the payload hashes the checks alone (claims, kinds and evidence paths), so Tried and narrative can grow while a node runs, or `pleach land` re-derives the payload from the file of record before it judges the composition. As it stands, a conductor who records a decision mid-run has to land by hand.
